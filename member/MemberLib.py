...
def safely_remove_members_from_group(member):
    members_backup = member.group_view
    response_to_client = constants.SUCCESS
    num_responses = 0
    members_to_remove = []
    respondents = []
    initial_group_size_including_leader = member.group_view.get_size()

    # member.agreement_socket.settimeout(30) # TODO @Amber add back in
    ...
    while num_responses < (initial_group_size_including_leader - 1):
        message, responder = member.agreement_socket.recvfrom(constants.RECV_BYTES)
        decoded_message = pickle.loads(message)
        if (decoded_message.get_message_type() is MessageType.MessageType.member_group_delete_response) and \
                (decoded_message.get_member_id() is not member.id):
            if decoded_message.get_data() == constants.AGREED:
    ...
    if num_responses == (initial_group_size_including_leader - 1):
        try:
            ...
            # FIXME Amber - must ensure that the main start_serving thread is killed
            ...
            # TODO will need to ensure that all the members of the group are able to roll back too
...
def safe_remove_members_from_group(member, respondents):
    empty_group_view = GroupView.GroupView() # This is what we want to end up with
    while member.group_view.get_size() > 1:
        ...
        member.agreement_socket.sendto(pickle.dumps(Message.Message(
           ...
           member.index_of_latest_uncommitted_log, member.index_of_latest_committed_log, empty_group_view)),
           (constants.MULTICAST_ADDRESS, constants.CONSENSUS_PORT))
    ...
    if member.group_view.contains(member.id):
